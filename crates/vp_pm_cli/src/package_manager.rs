...
/// Command argument resolution and execution live in `vp_pm_cli`.
#[derive(Debug)]
...
fn dev_engines_package_manager_conflict_message(
    field: &vp_shared::DevEngineField,
    resolution: &PackageManagerResolution,
) -> Option<Str> {
    let entries = field.entries();
    if entries.is_empty() {
...
mod tests {
    ...
    fn parse_dev_engines_pm_field(json: &str) -> vp_shared::DevEngineField {
        let pkg: vp_shared::PackageJson = serde_json::from_str(json).unwrap();
...
"#;
    ...
    fn test_detect_package_manager_priority_order_lock_over_config() {
        ...
        // pnpmfile.cjs and yarn.config.cjs are lower priority than lock files.
